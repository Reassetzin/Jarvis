...
export default function BrandTab() {
  ...
  const [postGoal] = usePersistentStore('post_goal', 1)
  ...
  function updateCount(platform: string) {
    ...
    setSocials(s => ({
      ...
      [platform]: {
        ...s[platform],
        history: [...(s[platform].history || []).slice(-4), s[platform].count],
        count: n,
      }
    }))
    setEditingTag(null)
  ...
  return (
    <div className="tab-scroll-notop" style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div className="card">
        ...
        <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#F3F4F6' }}>{brand.name}</div>
        {editingTagline ? (
          <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
            <input type="text" value={brand.tagline}
          ...
          <div onClick={() => setEditingTagline(true)} style={{ fontSize: '0.78rem', color: '#6B7280', marginTop: 4, cursor: 'pointer', padding: '4px 0' }}>
            ...
            return (
              <div key={p} style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#181818', border: '1px solid #222', borderRadius: 4, padding: '10px 12px' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#F3F4F6', fontVariantNumeric: 'tabular-nums' }}>
                    {s.count.toLocaleString()}
                  </div>
                  {delta !== 0 && (
                    <div style={{ fontSize: '0.65rem', color: delta > 0 ? '#22C55E' : '#EF4444' }}>
                      {delta > 0 ? '+' : ''}{delta} · prev {(s.history[s.history.length - 1] || 0).toLocaleString()}
                    </div>
                  )}
                </div>
                ...
                {editingTag === p ? (
                  <div style={{ display: 'flex', gap: 4 }}>
                    <input type="number" value={newCount} onChange={e => setNewCount(e.target.value)}
                      style={{ width: 80 }} placeholder="New count" autoFocus
                      onKeyDown={e => e.key === 'Enter' && updateCount(p)} />
                  ...
                  <button onClick={() => { setEditingTag(p); setNewCount(s.count.toString()) }}
                    className="btn-ghost" style={{ fontSize: '0.7rem', padding: '6px 10px' }}>Update</button>
                )}
              </div>
            )
          })}
      ...
      <div className="card">
        ...
        <div style={{ fontSize: '0.75rem', color: '#4B5563', marginBottom: 8, fontStyle: 'italic' }}>
        ...
        <textarea value={refText} onChange={e => setRefText(e.target.value)}
          rows={4} placeholder="Write your reflection..." style={{ marginBottom: 8, resize: 'vertical' }} />
        <button onClick={saveReflection} className="btn-amber">Save Reflection</button>
        {reflections.length > 0 && (
          <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ fontSize: '0.65rem', color: '#4B5563', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Past Reflections</div>
            {reflections.slice().reverse().slice(0, 3).map(r => (
              <details key={r.id} style={{ background: '#181818', border: '1px solid #222', borderRadius: 4, padding: '8px 12px' }}>
                <summary style={{ fontSize: '0.72rem', color: '#6B7280', cursor: 'pointer', listStyle: 'none' }}>
                  {r.date}
                </summary>
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          ...
          <span style={{ fontSize: '0.75rem', color: posts >= postGoal ? '#22C55E' : '#6B7280', fontWeight: 700 }}>{posts}/{postGoal}</span>
        </div>
        <button onClick={() => setPosts(p => p + 1)} className="btn-amber" style={{ marginBottom: 8 }}>
          Posted one ↑
        </button>
        {posts > 0 && (
          <button onClick={() => setPosts(p => Math.max(0, p - 1))} className="btn-ghost" style={{ width: '100%' }}>
            Undo
          </button>
        )}
      </div>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
          <input type="text" placeholder="Content idea..." value={ideaInput}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {ideas.map(idea => (
            <div key={idea.id} style={{
              display: 'flex', gap: 8, alignItems: 'center', background: '#181818',
              border: `1px solid ${idea.shipped ? '#166534' : '#222'}`, borderRadius: 4, padding: '8px 12px',
            }}>
              ...
              <span style={{ flex: 1, fontSize: '0.8rem', color: idea.shipped ? '#4B5563' : '#E5E7EB', textDecoration: idea.shipped ? 'line-through' : 'none' }}>
                {idea.text}
              </span>
              {idea.shipped && <span style={{ fontSize: '0.62rem', color: '#22C55E', fontWeight: 600 }}>SHIPPED</span>}
              <button onClick={() => setIdeas(is => is.filter(x => x.id !== idea.id))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#374151' }}>
                <X size={13} />
              </button>
            </div>
          ))}
          {ideas.length === 0 && <div style={{ fontSize: '0.78rem', color: '#374151', textAlign: 'center', padding: '8px 0' }}>No ideas yet.</div>}
        </div>
      </div>
    </div>
  )
