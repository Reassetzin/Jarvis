...
export default function GymTab() {
  ...
  function addSet(exId: string) {
    setExercises(es => es.map(e => e.id === exId ? { ...e, sets: [...e.sets, { reps: 0, weight: 0 }] } : e))
  }

  function updateSet(exId: string, si: number, field: 'reps' | 'weight', val: number) {
    setExercises(es => es.map(e => e.id === exId ? {
      ...e, sets: e.sets.map((s, i) => i === si ? { ...s, [field]: val } : s)
    } : e))
  }

  function removeSet(exId: string, si: number) {
    setExercises(es => es.map(e => e.id === exId ? { ...e, sets: e.sets.filter((_, i) => i !== si) } : e))
  }
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <select value={split} onChange={e => setSplit(e.target.value)} style={{ width: 'auto', fontSize: '0.75rem', padding: '4px 8px' }}>
            {DAY_SPLITS.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
          {exercises.map(ex => (
            <div key={ex.id} style={{ background: '#181818', border: '1px solid #222', borderRadius: 4 }}>
              <div onClick={() => setExpanded(x => x === ex.id ? null : ex.id)}
                style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', cursor: 'pointer' }}>
                <span style={{ flex: 1, fontSize: '0.85rem', fontWeight: 600, color: '#F3F4F6' }}>{ex.name}</span>
                ...
                <button onClick={e => { e.stopPropagation(); setExercises(es => es.filter(x => x.id !== ex.id)) }}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#374151' }}><X size={13} /></button>
              </div>
              {expanded === ex.id && (
                <div style={{ padding: '0 12px 12px' }}>
                  ...
                  <button onClick={() => addSet(ex.id)} className="btn-ghost" style={{ fontSize: '0.72rem', marginTop: 4 }}>
                    <Plus size={11} style={{ display: 'inline', marginRight: 4 }} />Add Set
                  </button>
                </div>
              )}
            </div>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
          <input type="text" placeholder="Exercise name..." value={exInput}
            onChange={e => setExInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addExercise()} style={{ flex: 1 }} />
        ...
        <textarea value={workoutNotes} onChange={e => setWorkoutNotes(e.target.value)}
          rows={2} placeholder="Workout notes..." style={{ marginBottom: 8, resize: 'none' }} />
        <button onClick={saveWorkout} disabled={exercises.length === 0} className="btn-amber">
      ...
      {history.length > 0 && (
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            ...
            <button onClick={() => setShowHistory(h => !h)} className="btn-ghost" style={{ fontSize: '0.7rem' }}>
          ...
          {(showHistory ? history : history.slice(0, 3)).map(log => (
            <details key={log.id} style={{ background: '#181818', border: '1px solid #222', borderRadius: 4, marginBottom: 6, padding: '8px 12px' }}>
              <summary style={{ fontSize: '0.78rem', color: '#9CA3AF', cursor: 'pointer', listStyle: 'none', display: 'flex', justifyContent: 'space-between' }}>
                <span>{log.date}</span>
                <span style={{ color: '#4B5563' }}>{log.exercises.length} exercises</span>
              </summary>
              <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
                {log.exercises.map(ex => (
                  <div key={ex.id} style={{ fontSize: '0.75rem', color: '#6B7280' }}>
                    <span style={{ color: '#E5E7EB', fontWeight: 600 }}>{ex.name}</span>
                    {' · '}{ex.sets.length} sets
