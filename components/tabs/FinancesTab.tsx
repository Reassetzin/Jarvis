...
export default function FinancesTab() {
  ...
  const monthlyBurn = subs.reduce((a, s) => {
    if (s.currency !== 'USD') return a
    return a + (s.period === 'monthly' ? s.amount : s.amount / 12)
  }, 0)
  ...
  return (
    <div className="tab-scroll-notop" style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div className="card">
        ...
        {[...crypto.map(a => ({ ...a, type: 'Crypto' })), ...otherAssets.map(a => ({ ...a, type: 'Other' }))].map(a => (
          <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderBottom: '1px solid #111' }}>
            ...
            <span style={{ fontSize: '0.78rem', fontWeight: 600, color: '#F3F4F6' }}>${a.amount.toLocaleString()}</span>
            <button onClick={() => {
              if (a.type === 'Crypto') setCrypto(c => c.filter(x => x.id !== a.id))
              else setOtherAssets(o => o.filter(x => x.id !== a.id))
            }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#374151' }}><X size={12} /></button>
          </div>
        ))}
        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
          <button onClick={() => setAddingAsset('crypto')} className="btn-ghost" style={{ flex: 1, fontSize: '0.72rem' }}>+ Crypto</button>
          <button onClick={() => setAddingAsset('other')} className="btn-ghost" style={{ flex: 1, fontSize: '0.72rem' }}>+ Other Asset</button>
        </div>
        {addingAsset && (
          <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button onClick={() => {
                if (!assetInput.label || !assetInput.amount) return
                const entry = { id: Date.now().toString(), label: assetInput.label, amount: parseFloat(assetInput.amount) }
                if (addingAsset === 'crypto') setCrypto(c => [...c, entry])
                else setOtherAssets(o => [...o, entry])
                setAssetInput({ label: '', amount: '' }); setAddingAsset(null)
              }} className="btn-amber" style={{ flex: 1 }}>Add</button>
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: '0.62rem', color: '#6B7280' }}>${yearlyBurn.toFixed(0)}/yr</div>
          </div>
        </div>
        {subs.map(s => (
          <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 0', borderBottom: '1px solid #111' }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '0.8rem', color: '#F3F4F6' }}>{s.name}</div>
        ...
        {!addingSub ? (
          <button onClick={() => setAddingSub(true)} className="btn-ghost" style={{ width: '100%', marginTop: 8 }}>
            <Plus size={13} style={{ display: 'inline', marginRight: 4 }} />Add Subscription
          </button>
        ) : (
          <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              <select value={subInput.currency} onChange={e => setSubInput(s => ({ ...s, currency: e.target.value }))} style={{ flex: 1 }}>
                {CURRENCIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <select value={subInput.period} onChange={e => setSubInput(s => ({ ...s, period: e.target.value as any }))}>
              <option value="monthly">Monthly</option>
              <option value="yearly">Yearly</option>
            </select>
            <input type="text" placeholder="Renewal date (e.g. Jan 15)" value={subInput.renewal} onChange={e => setSubInput(s => ({ ...s, renewal: e.target.value }))} />
            <div style={{ display: 'flex', gap: 6 }}>
              <button onClick={() => {
                if (!subInput.name || !subInput.amount) return
                setSubs(ss => [...ss, { id: Date.now().toString(), name: subInput.name, amount: parseFloat(subInput.amount), currency: subInput.currency, period: subInput.period, renewal: subInput.renewal }])
                setSubInput({ name: '', amount: '', currency: 'USD', period: 'monthly', renewal: '' }); setAddingSub(false)
              }} className="btn-amber" style={{ flex: 1 }}>Add</button>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <input type="text" placeholder="Item name..." value={orderInput} onChange={e => setOrderInput(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && orderInput.trim()) { setOrders(o => [...o, orderInput.trim()]); setOrderInput('') } }} style={{ flex: 1 }} />
          <button onClick={() => { if (orderInput.trim()) { setOrders(o => [...o, orderInput.trim()]); setOrderInput('') } }}
            style={{ background: '#F59E0B', color: '#000', border: 'none', borderRadius: 4, padding: '0 14px', cursor: 'pointer', fontWeight: 700 }}>+</button>
        </div>
      </div>
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div className="section-header" style={{ marginBottom: 0 }}>Weekly Haul Budget</div>
          <div style={{ fontSize: '0.9rem', fontWeight: 800, color: haulTotal > haulBudget ? '#EF4444' : '#22C55E' }}>
            CHF {haulTotal.toFixed(2)} / {haulBudget}
          </div>
        </div>
        <div style={{ marginBottom: 8 }}>
          <div style={{ fontSize: '0.65rem', color: '#6B7280', marginBottom: 4 }}>Budget threshold (CHF)</div>
        ...
        {haulItems.map(item => (
          <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid #111' }}>
            <span style={{ flex: 1, fontSize: '0.78rem', color: '#E5E7EB' }}>{item.name}</span>
        ...
        {!addingHaul ? (
          ...
          <div style={{ marginTop: 10, display: 'flex', gap: 6 }}>
            ...
            <button onClick={() => {
              if (!haulInput.name || !haulInput.cost) return
              setHaulItems(h => [...h, { id: Date.now().toString(), name: haulInput.name, cost: parseFloat(haulInput.cost) }])
              setHaulInput({ name: '', cost: '' }); setAddingHaul(false)
            }} style={{ background: '#F59E0B', color: '#000', border: 'none', borderRadius: 4, padding: '0 10px', cursor: 'pointer', fontWeight: 700 }}>+</button>
          </div>
        )}
      </div>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
          <button onClick={() => setWantCol('buy')} style={{ flex: 1, background: wantCol === 'buy' ? '#1a0a00' : 'transparent', border: `1px solid ${wantCol === 'buy' ? '#92400E' : '#333'}`, borderRadius: 4, padding: '8px', cursor: 'pointer', fontSize: '0.75rem', color: wantCol === 'buy' ? '#F59E0B' : '#6B7280', fontWeight: wantCol === 'buy' ? 700 : 400 }}>
            Still to Buy
          </button>
          <button onClick={() => setWantCol('future')} style={{ flex: 1, background: wantCol === 'future' ? '#0a0a1a' : 'transparent', border: `1px solid ${wantCol === 'future' ? '#1e3a5f' : '#333'}`, borderRadius: 4, padding: '8px', cursor: 'pointer', fontSize: '0.75rem', color: wantCol === 'future' ? '#3B82F6' : '#6B7280', fontWeight: wantCol === 'future' ? 700 : 400 }}>
            Future
          </button>
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
          {(wantCol === 'buy' ? wantsBuy : wantsFuture).map((w, i) => (
            <div key={i} className="tag" style={{ borderColor: wantCol === 'buy' ? '#92400E' : '#1e3a5f', color: wantCol === 'buy' ? '#F59E0B' : '#60A5FA' }}>
              {w}
              <button onClick={() => {
                if (wantCol === 'buy') setWantsBuy(ws => ws.filter((_, j) => j !== i))
                else setWantsFuture(ws => ws.filter((_, j) => j !== i))
              }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#374151', padding: 0 }}><X size={11} /></button>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          <input type="text" placeholder={wantCol === 'buy' ? 'Add to buy list...' : 'Add to future list...'} value={wantInput} onChange={e => setWantInput(e.target.value)}
            onKeyDown={e => {
              if (e.key === 'Enter' && wantInput.trim()) {
                if (wantCol === 'buy') setWantsBuy(w => [...w, wantInput.trim()])
                else setWantsFuture(w => [...w, wantInput.trim()])
                setWantInput('')
              }
            }} style={{ flex: 1 }} />
          <button onClick={() => {
            ...
            setWantInput('')
          }} style={{ background: wantCol === 'buy' ? '#F59E0B' : '#3B82F6', color: '#000', border: 'none', borderRadius: 4, padding: '0 14px', cursor: 'pointer', fontWeight: 700 }}>+</button>
        </div>
      </div>
    </div>
  )
